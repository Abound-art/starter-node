...
const run = (cfg: Config): Canvas => {
  ...
  const canvas = createCanvas(config.result_size, config.result_size);
  const ctx = canvas.getContext('2d');
  // const imgData = ctx.createImageData(config.result_size, config.result_size);
  const imgData = ctx.getImageData(0, 0, config.result_size, config.result_size);
